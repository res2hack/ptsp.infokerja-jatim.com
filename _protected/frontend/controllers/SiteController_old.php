...
//------------------------------------------------------------------------------------------------//
    ...
    {
      $lowongan_terbaru = (new \yii\db\Query())
        ->select('*')
        ->from('article')
        // ->innerJoin('ipk_perusahaan', 'ipk_lowongan.perusahaan_id=ipk_perusahaan.id')
        // ->where('tgl_penutupan_online >= NOW()')
        // ->andWhere(['ipk_lowongan.aktif' => '1'])
        // ->andWhere(['ipk_lowongan.sts_magang' => '0'])
        ->orderBy('id DESC')
        ->limit('10')
        ->all();

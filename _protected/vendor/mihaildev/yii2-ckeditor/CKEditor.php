...
class CKEditor extends InputWidget{
    ...
    public function init()
    {
        ...
        if(!isset($this->editorOptions['height']))
            $this->editorOptions['height'] = 200;
        if(!isset($this->editorOptions['preset']))
            $this->editorOptions['preset'] = 'basic';
    ...
    private function presetBasic(){
        $options['height'] = $this->editorOptions['height'];

        $options['toolbarGroups'] = [
						['name' => 'clipboard', 'groups' => ['mode','undo', 'selection', 'clipboard', 'doctools','templates', 'list', 'indent', 'align','basicstyles', 'cleanup','links', 'insert']],
        ];
        $options['removeButtons'] = 'Subscript,Superscript,Flash,Table,HorizontalRule,Smiley,SpecialChar,PageBreak,Anchor';
